...
public class EightQueens {
  public static void main(String[] args) {
    ...
    System.out.printf("%n\t** There %s %d possible solution%s **", solSize>1?"are":"is", solSize, solSize>1?"s":""); //formatting in case board is 1x1 (idk why the 1x1 board shows up as 3x3)

    // no, there is no error handling, so remember that index is actually index-1 so
    // that index "starts" at 1
    while (true) {
  ...
  //Solving row by row, so it's impossible to place 2 queens in the same row, don't need to check.
    ...
    for (int i = 0; i < bd; i++) {
      for (int j = 0; j < bd; j++) {
        System.out.printf("%s %s",
            board[i][j], (j + 1) == bd ? "\n\t\t" : ""); // if j == board dimension, go to next line (with tab for indenting)
      }
    }
    System.out.println(); // Fun fact: They do this println at the end in the book a couple of times, and
    // I believe this is because on zsh there is a "%" sign highlighting that this is a partial line.
